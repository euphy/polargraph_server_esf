...
void eeprom_loadSpoolSpec()
{
  ...
  EEPROM_readAnything(EEPROM_MACHINE_MM_PER_REV, in);
  if (in > 1) mmPerRev = in;
  Serial.print(F("Loaded mm per rev:"));
  Serial.println(mmPerRev);

  EEPROM_readAnything(EEPROM_MACHINE_STEPS_PER_REV, in);
  if (in > 1) stepsPerRev = in;
...
void eeprom_loadSpeed()
{
  ...
  float in;
  EEPROM_readAnything(EEPROM_MACHINE_MOTOR_SPEED, in);
  ...
  if (int(in) > 1) maxSpeed = in;
    
  EEPROM_readAnything(EEPROM_MACHINE_MOTOR_ACCEL, in);
  if (int(in) > 1) accel = in;
}
